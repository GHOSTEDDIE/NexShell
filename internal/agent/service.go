...
type OperationInput struct {
	...
	Operation      string `json:"operation" jsonschema:"description=observe service_status logs file_read file_write service_restart package_install or shell"`
	...
	verify, _ := utils.InferTool("verify_execution", "引用最后一次只读检查的执行编号和预期文本验证结果；有未确认变更时不能完成任务。", func(ctx context.Context, in *VerifyInput) (string, error) {
		...
		for _, r := range results {
			...
			if r.ID == in.ExecutionID {
				if remote.Mutates(r.Request.Operation) || r.Status != "succeeded" || !strings.Contains(r.Output, in.ExpectedText) {
