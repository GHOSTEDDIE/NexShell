...
- (instancetype)initWithGlfwWindow:(_GLFWwindow *)initWindow
{
    ...
    if (self != nil)
    {
        ...
        inputRect = NSMakeRect(0, 0, 1, 20);
        preeditLabel = [[NSTextField alloc] initWithFrame:inputRect];
        [preeditLabel setEditable:NO];
        [preeditLabel setSelectable:NO];
        [preeditLabel setBordered:NO];
        [preeditLabel setDrawsBackground:YES];
        [preeditLabel setBackgroundColor:[NSColor textBackgroundColor]];
        [preeditLabel setTextColor:[NSColor textColor]];
        [preeditLabel setHidden:YES];
        [self addSubview:preeditLabel];
...
- (void)keyDown:(NSEvent *)event
{
    ...
    // Composition owns editing/selection keys, including the committing key.
    // Resolve text first so starting a composition is also detected.
    const BOOL wasComposing = [self hasMarkedText];
    [self interpretKeyEvents:@[event]];
    if (!wasComposing && ![self hasMarkedText])
...
{
    return [self hasMarkedText] ? markedSelection : kEmptyRange;
}
     ...
     replacementRange:(NSRange)replacementRange
{
    ...
    [preeditLabel setStringValue:[markedText string]];
    [preeditLabel sizeToFit];
    [preeditLabel setFrameOrigin:inputRect.origin];
    [preeditLabel setHidden:![self hasMarkedText]];
                         ...
                         actualRange:(NSRangePointer)actualRange
{
    ...
    return [[self window] convertRectToScreen:[self convertRect:inputRect toView:nil]];
}
...
// Receives a rectangle in NSView coordinates from the focused Fyne widget.
- (void)setNexShellInputRect:(NSRect)rect
{
    inputRect = rect;
    [preeditLabel setFrameOrigin:rect.origin];
    [[self inputContext] invalidateCharacterCoordinates];
}
