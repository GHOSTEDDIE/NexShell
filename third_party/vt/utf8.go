...
func (e *Emulator) handleGrapheme(content string, width int) {
	...
	// A transport Write boundary is not a Unicode grapheme boundary. The
	// previous Write may already have rendered the base character; extend
	// that cell before advancing/wrapping when the next chunk joins it.
	if width == 0 || len(content) > 1 {
		px := x - 1
		if e.atPhantom {
			px = x
		}
		for px >= 0 {
			previous := e.scr.CellAt(px, y)
			if previous == nil {
				break
			}
			if previous.Width == 0 {
			...
			joined := previous.Content + content
			cluster, joinedWidth := ansi.FirstGraphemeCluster([]byte(joined), ansi.GraphemeWidth)
			if previous.Content != "" && len(cluster) == len(joined) {
				combined := previous.Clone()
				combined.Content, combined.Width = joined, joinedWidth
				e.scr.SetCell(px, y, combined)
				end := px + joinedWidth
				e.atPhantom = awm && end >= e.scr.Width()
				e.scr.setCursor(min(end, e.scr.Width()-1), y, false)
				return
			}
			break
		}
